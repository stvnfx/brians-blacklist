...
my $blacklist_url = `curl -s https://api.github.com/repos/stvnfx/brians-blacklist/releases/latest | grep browser_download_url | cut -d '"' -f 4`;
...
sub squidguard_auto_update {
    ...
    if (defined $file) {
      ...
      # get from net
        my $opt = '';
        $opt = "-s" if ! $interactive;
        $rc = system("curl -L -# -o $tmp_blacklists $opt $blacklist_url");
        if ($rc) {
